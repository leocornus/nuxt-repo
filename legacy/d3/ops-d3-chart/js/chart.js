...
function circleChart(margin, diameter, dataFile) {
    // map domain -1 to 5 to color range of hsl(152,80%,80%) to hsl(228,30%,40%)
    var color = d3.scale.linear()
        .domain([-1, 5])
        .range(["hsl(152,80%,80%)", "hsl(228,30%,40%)"])
        .interpolate(d3.interpolateHcl);

    // make pack layout for hierarchal layouts in D3, (circles within circles)
    ...
    d3.json(dataFile, function(error, root) {
        ...
        // selectAll says "map nodes[] to <circle>"; create <circle> for <g>
        var circle = svg.selectAll("circle")
            .data(nodes)
            // .enter() binds new data to new element (<circle> in this case)
            .enter().append("circle")
        ...
        svg.selectAll("symbol")
            ...
            .enter().append("symbol")
              ...
              // 'viewBox' to ensure entire image shows in different circle size
              .attr("viewBox", function(d) {
                  // viewBox sized to display entire image in fixed size parent svg
        ...
        // display <symbol> using <use> (this way the chart works in chrome/IE)
        var image = svg.selectAll("use")
            .data(nodes)
            .enter().append("use")
              .attr("xlink:href", function(d, i) { return "#img-" + i; });
        ...
        // to move <circle>, <text>, <use> according to zoom (active circle)
        ...
        function zoom(d) {
            ...
            .filter(function(d) { return d.parent === focus || this.style.display === "inline" || d.parent === focus0; })
            ...
            transition.selectAll("use")
                .style("opacity", function(d) { return focus.depth === 0 ? 1 : 0.4; });
        }
        ...
        function zoomTo(v) {
            ...
            svg.selectAll("use").each(function(d, i) {
                // get current transform info and translate it to circle centre
                transformInfo = d3.transform(d3.select(this).attr("transform"));
            ...
            svg.selectAll("text").each(function(d, i) {
                // get current transform info and translate it to circle centre
                console.log(this);
                var transformInfo = d3.transform(d3.select(this).attr("transform"));
                //var parentTransform = d3.transform(d3.select(this.parent).attr("transform"));
                var height = d3.select(this).attr("height");
                var width = d3.select(this).attr("width");
                var x = transformInfo.translate[0] - (width / 2);
                //var y = transformInfo.translate[1] - (height / 2) - parentTransform.translate[1];
                var y = transformInfo.translate[1] - (height / 2);
                //var y = - 20.0 - (height / 2);
                // set new transform translation
                d3.select(this)
...
circleChart(20, 960, 'data/flare.json');
